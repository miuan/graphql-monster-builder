...
import {
  ...
  describe('scan', () => {
    ...
    it('array with required relation is not supported', async () => {
        ...
        expect(models[0].members[1].relation.error).toEqual(`Line 4: Array field 'model2' with relation to Model2 (as many) can't be required! Only required relations to ONE are supported`);
