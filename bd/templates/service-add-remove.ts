...
export const _LOWER_NAME_AddTo_RELATION_NAME_ = (entry) => {
    ...
    return async (_RELATED_PARAM_NAME_1_, _RELATED_PARAM_NAME_2_) => {
      if (entry.hooks && entry.hooks.services['beforeUserRoleAddTo_RELATION_NAME_']) {
        await entry.hooks.services['beforeUserRoleAddTo_RELATION_NAME_'](entry, { _RELATED_PARAM_NAME_1_, _RELATED_PARAM_NAME_2_ });
      }
      ...
      if (entry.hooks && entry.hooks.services['afterUserRoleAddTo_RELATION_NAME_']) {
        await entry.hooks.services['afterUserRoleAddTo_RELATION_NAME_'](entry, { _LOWER_NAME_Related, _RELATED_MODEL_NAME_Related, _RELATED_PARAM_NAME_1_, _RELATED_PARAM_NAME_2_ });
      }
